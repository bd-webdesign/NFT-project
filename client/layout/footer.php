<div class="footer">
    <p class="flex-center">Nature Fairy Tales &copy; <?php echo date("Y"); ?></p>
    <ul>
        <div class="social flex-center">
            <li><a href="https://www.instagram.com/nftales2022/"><i class="fab fa-instagram"></i></a></li>
            <li><a href="https://twitter.com/nature_fairy22"><i class="fab fa-twitter"></i></a></li>
            <li>
                <a href="https://opensea.io/collection/nature-evoking-emotions"><img src="/NFT/assets/img/icons/opensea.svg" class="opensea" width="40px" height="40px"></a>
            </li>
        </div>
    </ul>
    <p class="flex-center">Powered by BND design</p>

</div>

</div>
<script src="https://kit.fontawesome.com/bf492b8f48.js" crossorigin="anonymous"></script>
<script src="/NFT/assets/libs/jquery-3.6.0.js"></script>
<script type="text/javascript" src="/NFT/assets/js/js.js?v=<?php echo time(); ?>"></script>
<script type="text/javascript" src="/NFT/assets/js/collection.js?v=<?php echo time(); ?>"></script>
</body>

</html>